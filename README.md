...
├── data/
│   ├── buy_status_GL.csv              # WINNER 購入履歴・結果管理（グループステージ）
│   ├── buy_status_TM.csv              # WINNER 購入履歴・結果管理（R32〜決勝）
│   ├── raw/
│   │   ├── match/                     # 試合結果データ
...
flowchart TD
    ...
    subgraph S5["⑤ 購入記録・自動精算"]
        WEVOUT -. 購入を記録 .-> BUY[("buy_status_GL.csv<br/>buy_status_TM.csv")]
        BETREC -. 購入を記録 .-> BUY
        OEVOUT -. 購入を記録 .-> BUY
...
**購入と精算** — 実際に購入したら `buy_status_GL.csv`（グループステージ）または `buy_status_TM.csv`（R32以降）に1行記録し、`settle_bets_2026.py` が
...
【出力】対戦カード・モデル予測(λ, 1X2%)、EV>=1.0の選択肢の表(オッズ/確率/EV/推奨額)、
       ...
       購入したら buy_status_GL.csv（GL）または buy_status_TM.csv（TM以降）に1行記録し、結果確定後は最終オッズで result_amount を精算する。
```
...
| ラウンド | ベット数 | 的中 | 的中率 | 投資額 | 回収額 | **損益** |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| グループステージ (GL) | 24 | 5 | 20.8% | 14,600円 | 59,300円 | **+44,700円** |
| R32 | 3 | 0 | 0% | 7,600円 | 0円 | -7,600円 |
| R16 | 5 | 0 | 0% | 5,000円 | 0円 | -5,000円 |
| QF | 2 | 0 | 0% | 3,900円 | 0円 | -3,900円 |
| SF | 1 | 0 | 0% | 600円 | 0円 | -600円 |
| 3位決定戦 | 1 | 0 | 0% | 400円 | 0円 | -400円 |
| 決勝 | 見送り | — | — | — | — | — |
| **合計** | **36** | **5** | **13.9%** | **32,100円** | **59,300円** | **+27,200円（ROI +84.7%）** |

### GL 5的中の内訳

| 試合 | 選択 | オッズ | 投資 | 回収 | 利益 |
|:---|:---|:---:|:---:|:---:|:---:|
| Portugal vs DR Congo | 1-1 | 9.1 | 1,000円 | 9,100円 | +8,100円 |
| England vs Croatia | England その他(4+) | 11.1 | 1,400円 | 15,540円 | +14,140円 |
| Switzerland vs Bosnia and Herzegovina | Switzerland その他(4+) | 6.7 | 1,800円 | 12,060円 | +10,260円 |
| Canada vs Qatar | Canada その他(4+) | 8.3 | 2,000円 | 16,600円 | +14,600円 |
| Netherlands vs Sweden | Netherlands その他(4+) | 7.5 | 800円 | 6,000円 | +5,200円 |

### モデルの勝者予測精度（ノックアウトラウンド）

| ラウンド | 予測（フェイバリット） | 実結果 | 正否 |
|:---:|:---|:---|:---:|
| QF M97 | France 60.2% | France 2-0 Morocco | ✓ |
| QF M98 | Spain 63.4% | Spain 2-1 Belgium | ✓ |
| QF M99 | England 54.1% (away) | England 1-2 Norway → England勝 | ✓ |
| QF M100 | Argentina 63.5% | Argentina 3-1 Switzerland | ✓ |
| SF M101 | Spain 35.1% (僅差) | Spain 2-0 France | ✓ |
| SF M102 | Argentina 38.3% | Argentina 2-1 England | ✓ |
| 3位 M103 | France 46.0% | England 6-4 France | ✗ |
| 決勝 M104 | Spain 43.8% | 2026/07/20 開催 | — |

QF・SF計6試合で**6/6正解**（3位決定戦はフランス有利予測がはずれ）。
大会開幕前シミュレーション（1万回モンテカルロ）で**スペイン23.04%・アルゼンチン11.83%**を1・2位予測し、
実際に両チームが決勝進出した。
...
> **大会終了後注記（2026-07-20）**: 大会全体の最終収支・総括は「[2026年大会 最終収支・総括](#2026年大会-最終収支総括)」セクションを参照。
...
## 2026年大会 運用知見（大会全体・確定実績）

2026年大会（全36ベット・グループステージ〜3位決定戦）の確定実績から得られた知見。

### 的中の必要条件（大会全体の結論）

| 条件 | 根拠（大会全体） |
|:---|:---|
| **確率10%以上** | 10%未満: 0勝、10%以上: 5勝31敗 |
| **格上チームのその他（大量得点）** | 5的中のうち4件が「強豪チームのその他(4得点以上)」 |
| **グループステージ** | GL5勝/TM0勝。試合数の母数効果がEV収束に必須 |

この2条件を**両方**満たした場合のみ4勝1敗。それ以外は1勝30敗。

**的中した「強豪その他」の共通点:**

| 的中 | 選択 | 結果 | 構図 |
|:---|:---|:---:|:---|
| England vs Croatia | England その他(4+) | 4-2 | 強豪が明確に上回る相手に大量得点 |
| Switzerland vs Bosnia and Herzegovina | Switzerland その他(4+) | 4-1 | 同上 |
| Canada vs Qatar | Canada その他(4+) | 6-0 | 開催国 vs 格下の圧倒 |
| Netherlands vs Sweden | Netherlands その他(4+) | 5-1 | 強豪が守備の薄い相手に大量得点 |
...
| Netherlands vs Japan | Netherlands その他 | 2-2 | 日本が2点差を追いつく。応援バイアスによるオッズ歪みも関与の可能性 |

**TM（R32〜SF）: 全12ベット不的中の構造的理由**

TMのベットは主に「強豪チームのその他（スコア外勝利）」だったが、実際の結果は
1-2・2-1・0-1 など特定スコアに収まり取れなかった。18択スコア予測では試合数が少なく
ボラティリティが支配的になる。勝者予測の精度（QF+SF 6/6正解）とスコア予測の精度は別物。

**引き分け・特定スコア予測の戦績（全大会）:** 1勝10敗。
Portugal 1-1 の1件のみ的中。スコア行列の較正は統計的には正しいが、個別試合では運の支配が大きい。

### GLとTMの収益構造の違い

| 要因 | GL（24試合） | TM（12試合） |
|:---|:---|:---|
| 試合数 | 多い（EV収束の母数あり） | 少ない（ボラティリティ支配） |
| 1試合あたりの投資額 | 小〜中（200〜2,000円） | 中〜大（400〜3,200円） |
| 結果 | +44,700円（ROI +306%） | -17,500円（ROI -100%） |
| 教訓 | GLのEV戦略は継続価値あり | TMはEV閾値1.15〜1.20に引上げ、または見送りが合理的 |

### 低確率ロングショットの扱い（確定結論）

確率5%以下のEV超過は**見送りが正解**（大会全体で0勝）:
- EV高騰はオッズ80倍超の裾であり、モデル確率自体の信頼区間が広い
- 「当たる必要条件（確率10%以上）」を満たさない

### 最終ROIの解釈

大会全体の実績ROI: **+84.7%**（投資32,100円、回収59,300円、利益+27,200円）。

- **期待ROI（バックテスト基準）**: 100〜200%程度
- **実績ROI +84.7%**: GLで大きく上振れ、TM全滅で一部相殺された最終着地点
- 利益の源泉はGL5的中（全59,300円）。TM以降は投資額の回収ゼロ
- 大会全体で見ると「GLで稼いでTMで一部返却」という典型的なWINNER高ボラ収益パターン