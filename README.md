...
2026年グループステージ序盤（〜6/21、17ベット・5的中）の実績から見えてきた経験則。
サンプルが少なく確定的ではないが、今後の判断の補助として記録する。

### 的中の必要条件（現時点の経験則）

| 条件 | 根拠 |
|:---|:---|
| **確率10%以上** | 確率10%未満: 0勝7敗。10%以上: 5勝5敗 |
| **格上チームのその他（大量得点）** | 5的中のうち4件が「強豪チームのその他(4得点以上)」 |

この2条件を**両方**満たした場合のみ4勝1敗（80%）。それ以外は2勝13敗。

**的中した「強豪その他」の共通点:**

| 的中 | 選択 | 結果 | 構図 |
|:---|:---|:---:|:---|
| England vs Croatia | England その他 | 4-2 | 強豪が明確に上回る相手に大量得点 |
| Switzerland vs Bosnia and Herzegovina | Switzerland その他 | 4-1 | 同上 |
| Canada vs Qatar | Canada その他 | 6-0 | 開催国 vs 格下の圧倒 |
| Netherlands vs Sweden | Netherlands その他 | 5-1 | 強豪が守備の薄い相手に大量得点 |
...
| 試合 | 選択 | 結果 | 解釈 |
|:---|:---|:---:|:---|
| Qatar vs Switzerland | Switzerland その他 | 1-1 | Qatarが守備組織で引き分けに持ち込んだ |
| Netherlands vs Japan | Netherlands その他 | 2-2 | 日本が2点差を追いつく。WINNER日本人ユーザーの応援バイアスによるオッズ歪みも関与の可能性 |

→ **守備的ゲームプランを取るチームが相手の場合は「格上その他」でも注意**。
モデルはチームの強さは捉えるが「この試合でどう戦うか」は捉えにくい。
Qatar・日本のような組織的守備を得意とするチームへの格上の「その他」は慎重に判断する。

**引き分け・特定スコア予測の戦績:** 1勝4敗（例外: Portugal 1-1 の的中1件のみ）。
スコア行列の較正精度は統計的に担保されているが、1試合ごとの結果は運の影響が大きい。

### 低確率ロングショットの扱い

確率5%以下のEV超過（例: Iran その他 EV1.68, 確率2%）は**見送りが妥当**:
- 今大会の残り試合数では当たりを引く期待回数が1を大きく下回る
- EV高騰はオッズ80倍超の裾であり、モデル確率自体の信頼区間も広い
- これまでの「当たる必要条件（確率10%以上）」を満たさない

### ROIの解釈

グループステージ序盤（〜6/21）の実績ROI: **494%**（期待値ベース: **135%**）。
上振れ額: 約43,000円。

- **期待ROI 135%** = 今後の各ベットで長期的に期待できる回収率
- **実績ROI 494%** = 高倍率4試合連続的中による上振れ。利益の大部分は運
- トーナメント終了時点でも期待ROIより高い水準が続く見込み（過去の利益は確定済みのため）
- 残り試合は「47,000円の貯金の上に期待ROI135%のゲームを積み上げる」構造