...
def main():
    ...
    FINAL_FIXTURES = [
        # (home, away, date, FIFA_match_no)
        ('France',     'England',    '2026-07-18', 103),  # 3rd place
        ('Spain',      'Argentina',  '2026-07-19', 104),  # Final
    ]

    print("\n" + "=" * 90)
    print(f"{'Final':<6} {'Home':<25} {'Pred':^7} {'Away':<25} "
          f"{'λH':>5} {'λA':>5}  {'Home%':>6} {'Draw%':>6} {'Away%':>6}  {'MostLikelyProb':>14}")
    print("=" * 90)

    for home, away, date, fifa_no in FINAL_FIXTURES:
        res = predict_match_score(home, away, lambda_cache,
                                  cls_proba_cache=cls_proba_cache,
                                  cls_blend=args.cls_blend)
        score_str = f"{res['pred_a']}-{res['pred_b']}"
        print(f" M{fifa_no:<3} {home:<25} {score_str:^7} {away:<25} "
              ...
              f"{res['pred_score_prob']:>5.1f}%")
        records.append({
            'group': f'Final_M{fifa_no}', 'home_team': home, 'away_team': away,
            'pred_home_score': res['pred_a'],
            'pred_away_score': res['pred_b'],
            'pred_score_str':  score_str,
            'pred_score_prob': res['pred_score_prob'],
    ...
    group_count = df_out[~df_out['group'].str.startswith(('R32', 'R16', 'QF', 'SF', 'Final'))].shape[0]
    ...
    final_count = df_out[df_out['group'].str.startswith('Final')].shape[0]
    print(f"\nSaved {group_count} group stage + {r32_count} R32 + {r16_count} R16 + {qf_count} QF + {sf_count} SF + {final_count} Final predictions to: {output_path}")
